...
import { IconCategory, icons } from '../../../shared/icons/icons'
...
const ParagraphContent = ({ getData, updateData }: ParagraphContentProps) => {
  const [iconFilter, setIconFilter] = useState('')

  const getFilteredCategories = (): IconCategory[] => {
    if (!iconFilter.trim()) return Object.keys(icons) as IconCategory[]
    const normalizedFilter = iconFilter.toLowerCase().trim()
    return (Object.keys(icons) as IconCategory[]).filter((category) => {
      const categoryName = category.toLowerCase().replace('_', ' ')
      const labelName = CATEGORY_LABELS[category].toLowerCase()
      return categoryName.includes(normalizedFilter) || labelName.includes(normalizedFilter)
    })
  }
  ...
  return (
    <>
      ...
      <div className="field">
        <label className="field-label">Insert Icon</label>
        ...
        <div className="icons-section">
          {filteredCategories.length === 0 ? (
            <p className="no-icons">No categories found</p>
          ) : (
            filteredCategories.map((category) => (
              <div key={category} className="icon-category">
                <div className="icon-category-label">{CATEGORY_LABELS[category]}</div>
                <div className="icons-grid">
                  {icons[category].map((icon, idx) => (
                    <button
                      key={`${category}-${idx}`}
                      className="icon-btn"
                      title={`Insert ${icon}`}
                      onClick={() => {
                        const currentContent = getData('content')
                        updateData('content', currentContent + icon)
                      }}
                    >
                      {icon}
                    </button>
  ...
  const renderContentFields = () => {
    switch (elementType) {
      ...
      case WidgetType.Paragraph:
        return <ParagraphContent getData={getData} updateData={updateData} />
